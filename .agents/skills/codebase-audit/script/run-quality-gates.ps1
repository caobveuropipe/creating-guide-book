...
=======
  [switch]$AsJson,
  # Helper nay gia dinh repo Node/JS co package.json. Package manager lay tu
  # .agents/rules/project-gates.md muc G6 cua du an; pnpm chi la mac dinh.
...
=======
    Command = "$PackageManager run $Step"
>>>>>>> 301089a0141b6b491eb86952afcb843492685d91
    Status = "planned"
    ExitCode = $null
    DurationSec = $null
...
=======
    & $PackageManager run $Entry.ScriptName | Out-Host
    $exitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } else { 0 }
    if ($exitCode -ne 0) {
      $status = "failed"
      $reason = "$PackageManager returned exit code $exitCode."
>>>>>>> 301089a0141b6b491eb86952afcb843492685d91
    }
  } catch {
    $status = "failed"
