...
- Với audit toàn repo, đọc tối thiểu theo thứ tự:
  1. `.agents/CONTEXT.md`
  2. `.agents/KNOWLEDGE_BASE.md`
  3. `.agents/PROJECT_STRUCTURE.md`
  4. `README.md`
  5. root `package.json`
<<<<<<< HEAD
...
=======
  6. `.agents/rules/project-gates.md` — §G1 canonical source, §G4 vùng cấm ghi, §G6 lệnh chuẩn
  7. manifest của từng workspace/package con — đọc từ repo, đừng giả định tên
  8. test/config/CI files liên quan
>>>>>>> 301089a0141b6b491eb86952afcb843492685d91
...
- Khi nghi vấn liên quan rule nghiệp vụ hoặc access control, bắt buộc đối chiếu:
  - `.agents/business/data/PERMISSION_MATRIX.md`
  - `.agents/business/data/STATE_MACHINES.md`
  - `docs/business-flows/00-MASTER-INDEX.md`
=======
- Khi nghi vấn liên quan rule nghiệp vụ hoặc access control, bắt buộc đối chiếu tài liệu nghiệp vụ đang có đường đọc — tra bảng điều hướng ở `.agents/CONTEXT.md`, thường là `.agents/business/*` và `.agents/architecture/*`. Nếu một tài liệu nghiệp vụ chỉ còn hiệu lực **một phần**, đọc banner đầu file trước khi trích nó làm evidence.
>>>>>>> 301089a0141b6b491eb86952afcb843492685d91
- Khi đã có `.agents/testing/*.md`, phải đối chiếu test case docs với coverage thực tế để phát hiện lỗ hổng kiểm thử.
...
=======
Nếu audit cần thêm test runner mới, thư viện UI automation, mock server, browser binary, hoặc test config mới, phải dừng lại và hỏi user trước.

Mẫu giải thích bắt buộc:
- Mục đích kỹ thuật của dependency/tooling mới
- File sẽ bị ảnh hưởng: manifest dependency, lockfile, config test, script CI/local
>>>>>>> 301089a0141b6b491eb86952afcb843492685d91
- Tác động vào repo:
  - lockfile churn
...
2. Đặt tên artifact rõ mục đích, ví dụ `audit-*`, `*.audit.test.ts`, hoặc thư mục tạm dưới `.agents/tmp/codebase-audit/`.
...
=======
4. Khi test mới đã ổn định, chạy quality gate đầy đủ của repo — lệnh lấy từ `.agents/rules/project-gates.md` §G6, không tự đoán:
   - lint
   - typecheck
   - build
   - các test hiện có ở từng workspace
   ...
   Nếu §G6 có danh sách lệnh AI không được tự chạy, tôn trọng danh sách đó và ghi vào báo cáo là gate đó chưa chạy được, kèm lệnh cần user chạy.
5. Nếu repo phù hợp với helper hiện có, ưu tiên dùng `script/run-quality-gates.ps1 -Mode Run` cho baseline gate rồi chạy thêm các custom/generated tests nằm ngoài baseline đó. Helper này **giả định repo Node/JS có `package.json`**; package manager truyền qua `-PackageManager` theo §G6. Repo khác stack thì bỏ qua helper và chạy trực tiếp lệnh ở §G6.
6. Nếu repo không có script chuẩn cho một gate, suy ra lệnh tương đương từ manifest của repo, nói rõ lý do, và đề xuất bổ sung lệnh đó vào §G6.
>>>>>>> 301089a0141b6b491eb86952afcb843492685d91
